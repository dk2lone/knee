...
```
cloud/train.py     setup | encoder | import | sweep | arm | full
...
cloud/data.py      probe | bulk | land | finish         (rate measurement; land is retired)
```

**The corpus never touches a Volume.** A Volume cannot hold 570 GB of DICOM — `unzip`
exits 50 partway through and afterwards even a 90 MB write fails (#32). It lives on
ephemeral disk, which dies with the container, so `--mode sweep` extracts once and runs
every arm inside that one container, memoising the pixel cache in RAM between them. Only
members, `oof.csv` and `manifest.json` reach the Volume.

**`--mode encoder` validates a backbone with no corpus present**, on a CPU container in a
minute. Four encoders pass at 336 px:

| encoder | params | trainable at unfreeze 6 | licence |
|---|---:|---:|---|
| DINOv2-small | 22.3M | 10.9M | Apache |
| DINOv2-base | 87.0M | 42.9M | Apache |
| RAD-DINO | 87.0M | 42.9M | **MIT** |
| BioMedCLIP | 86.4M | 42.9M | **MIT** |

**Do not launch anything against the `knee-train` app while a sweep runs.** A `modal run`
cancels the app's other running inputs; a one-minute encoder check killed a six-hour sweep
37.9 GB into its download. Monitor with `modal app logs <id>`, which is read-only.

**Neither MIT encoder is on Kaggle.** A scored kernel has no internet, so a RAD-DINO or
BioMedCLIP member cannot be rebuilt there until its weights are published as a Kaggle
dataset — MIT permits that. It is a prerequisite for whichever encoder wins, not for the
sweep that decides which does.
